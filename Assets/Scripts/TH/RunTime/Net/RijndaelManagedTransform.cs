...
namespace Net
{
    ...
    public sealed class RijndaelManagedTransform2 : ICryptoTransform
    {
        ...
        public RijndaelManagedTransform2(byte[] rgbKey,
                                           CipherMode mode,
                                           byte[] rgbIV,
                                           int blockSize,
                                           int feedbackSize,
        ...
        public int TransformBlock(byte[] inputBuffer, int inputOffset, int inputCount, byte[] outputBuffer, int outputOffset)
        {
            ...
            if (inputCount <= 0 || (inputCount % InputBlockSize != 0) || (inputCount > inputBuffer.Length)) throw new ArgumentException(("Argument_InvalidValue"));
        ...
        public byte[] TransformFinalBlock(byte[] inputBuffer, int inputOffset, int inputCount)
        {
            ...
            else
            {
                if (inputCount % InputBlockSize != 0)
                    throw new CryptographicException(("Cryptography_SSD_InvalidDataSize"));
                                       ...
                                       bool fLast)
        {
            if (inputBuffer.Length < inputOffset + inputCount)
                throw new CryptographicException(("Cryptography_InsufficientBuffer"));
                    ...
                    fixed (int* T = s_T)
                    {
                        fixed (int* TF = s_TF)
                        {
                            ...
                            for (int blockNum = 0; blockNum < iNumBlocks; ++blockNum)
                            {
                                ...
                                {
                                    for (int i = 0; i < m_blockSizeBytes; ++i)
                                    {
                                        ((byte*)work)[i] = inputBuffer[i + workBaseIndex];
                                    }
                                ...
                                fixed (int* TF = s_TF)
                                {
                                    fixed (int* iT = s_iT)
                                    {
                                        fixed (int* iTF = s_iTF)
                                        {
                                            ...
                                            if (inputCount == 0)
                                                throw new CryptographicException(("Cryptography_PKCS7_InvalidPadding"));
        ...
        public unsafe int DecryptBytes(byte[] inputBuffer,
                                       int inputOffset,
                                       int inputCount,
                                       byte[] outputBuffer,
                                       int outputOffset)
                                ...
                                fixed (int* TF = s_TF)
                                {
                                    fixed (int* iT = s_iT)
                                    {
                                        fixed (int* iTF = s_iTF)
                                        {
                                            ...
                                            // this is the last block, remove the padding.
                                            int padSize = padSize = outputBuffer[outputOffset + inputCount - 1];
                                            return inputCount - padSize;
                                        }
                                    }
                                }
